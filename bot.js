...
async function getOrdersByPhone(phone) {
    ...
    try {
        ...
        // **Telefon numarasını eşleştirerek ilgili siparişleri getiriyoruz**
...
// Siparişler için interaktif mesaj gönderme
async function sendOrderInteractiveMessage(to, order) {
    ...
    const productName = (order.orderItems.length > 0 && order.orderItems[0].product.name) ? order.orderItems[0].product.name : "Ürün bilgisi yok";
...
async function sendWhatsAppMessage(to, message) {
    const url = `https://graph.facebook.com/v17.0/${PHONE_NUMBER_ID}/messages`;
    const data = { messaging_product: "whatsapp", to: to, type: "text", text: { body: message } };
